...
- **Theme.** The shared `Theme` resource lives at `resources/theme/base_theme.tres` — apply it on the `Book` Panel (the first `Control` ancestor of `LeftPage`/`RightPage`). This lets font, palette, and any future overrides cascade to all tab content, including subtrees that `notebook.gd` reparents in at runtime. Do not define a separate per-notebook theme — share `base_theme.tres` so the notebook and future dialog boxes inherit an identical look for free. See `docs/refactors/pixel-art-purist-size-and-theme.md` §4 and §6.2.
  - **Theme application status (as of issue #47):** `base_theme.tres` is populated with the monogram font, default font size 8, nine named palette colors, and a `Heading` Label variation at size 16. It is set on the `Book` Panel (the notebook's top-level `Control`) so it cascades to all descendants. `Panel/styles/panel` and `PanelContainer/styles/panel` StyleBoxFlat entries now apply the `paper_bg` / `paper_bg_shadow` page background with a `frame_line` border, and `Label/colors/font_color` now applies `ink` as the default text color — so page backgrounds and body text render with the palette. `Button` theming (styleboxes and all font states) remains deferred — see issue #48. Additionally, tab-title Labels and the close-hint Label do **not exist yet** in the scene tree; those nodes are planned/deferred to a separate task and should receive the `Heading` variation (tab titles) or remain at size 8 (close hint) once added.
- **Pixel-perfect rendering.** The project uses a **320×180** viewport with `stretch=canvas_items` (not `viewport`). The `canvas_items` mode keeps fonts crisp at any window scale, at the cost of requiring 2D snap settings (`snap_2d_transforms_to_pixel`, `snap_2d_vertices_to_pixel`) to prevent sub-pixel shimmer on sprites. The stretch aspect is `keep` (letterbox on odd shapes) and scale mode is `integer` (upscales only by whole numbers). See `docs/refactors/pixel-art-purist-size-and-theme.md` §2 for the full rationale and project.godot diff.
- **Font sizes.** The project uses **two sizes only** — body **8** and heading **16** — both exact divisors/multiples of monogram's 16-unit design grid, so both stay pixel-perfect. Mapping:
  - Size **16** (use the `Heading` Label theme type variation): tab titles, quest titles, Story names, dialog speaker names.
  - Size **8** (the base theme default — no override needed): inventory rows, weight readout, quest descriptions, settings labels, the close hint (`"I to open/close • Esc to close or cancel"`), objective bullets.
  - Differentiate secondary/muted text (e.g. `Weight: 12.4 / 30.0` denominator, completed quest rows, hint text) using the `ink_muted` palette color rather than a smaller font size. Both the label and its muted counterpart stay at size 8; color alone carries the hierarchy signal.
  - Do **not** introduce size 12 or any other non-multiple — it will render with uneven glyph stems at this font's native grid.
- **Layout at 320×180.** A notebook page is genuinely small at this resolution. With 8 px body text, expect roughly **18–20 short rows** per page maximum before content overflows. The inventory and quests `ScrollContainer`s (already specified in §5.2 and §5.4) are therefore **load-bearing, not optional** — long item lists and long quest lists must scroll; do not try to fit everything in a fixed-height panel. For two-page spreads, budget approximately **140×150 px per page** after the book-frame margins are accounted for (the full spread is ~280×160 inside a 320×180 frame).
