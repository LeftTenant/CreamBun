...
- **Theme.** The shared `Theme` resource lives at `resources/theme/base_theme.tres` — apply it on the `Book` Panel (the first `Control` ancestor of `LeftPage`/`RightPage`). This lets font, palette, and any future overrides cascade to all tab content, including subtrees that `notebook.gd` reparents in at runtime. Do not define a separate per-notebook theme — share `base_theme.tres` so the notebook and future dialog boxes inherit an identical look for free. See `docs/refactors/pixel-art-purist-size-and-theme.md` §4 and §6.2.
  - **Theme application status (as of pixel-art-purist Slices 1–5):** `base_theme.tres` is populated with the monogram font, default font size 8, nine named palette colors, and a `Heading` Label variation at size 16. It is set on the `Book` Panel (the notebook's top-level `Control`) so it cascades to all descendants. However, visible application of the palette colors to individual controls (e.g. `paper_bg` as the page background, `ink` as the body font color) has been **deferred to a later per-control styling phase** — the base theme establishes the defaults, but page background/body-color theming is not yet applied to specific nodes. Additionally, tab-title Labels and the close-hint Label do **not exist yet** in the scene tree; those nodes are planned/deferred to a separate task and should receive the `Heading` variation (tab titles) or remain at size 8 (close hint) once added.
