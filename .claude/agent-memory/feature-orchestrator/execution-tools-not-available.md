...
**How to apply:** when a task (even a "resume, just run the loop" task) says "have godot-coder do X" or "run tests via the godot mcp server", do NOT attempt to call those tools or edit files — they will not exist. Instead write a self-sufficient `slices.md` (the parent's `/implement-feature` flow fills its build-loop prompts directly from it) and hand control back to the parent with a slice summary. Do not thrash searching for tools that are absent; verify once via ToolSearch, then deliver the breakdown.
