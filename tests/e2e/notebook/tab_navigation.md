...
Verifies that while the notebook is open, `Tab` and `Shift+Tab` cycle through
...
### Step 4: Tab — SETTINGS (3) → SESSIONS (4)
- Press action: `ui_focus_next`
- Wait: 5 frames
...
### Step 5: Tab wraps SESSIONS (4) → INVENTORY (0)
- Press action: `ui_focus_next`
- Wait: 5 frames
- Assert: node property `Notebook._current_tab` == `0` (INVENTORY)
- Assert: node property `Notebook.visible` == `true`  (no accidental close)

### Step 6: Shift+Tab wraps backward INVENTORY (0) → SESSIONS (4)
- Press action: `ui_focus_prev`
- Wait: 5 frames
- Assert: node property `Notebook._current_tab` == `4` (SESSIONS)
